...
class SRData(data.Dataset):
    ...
    def _get_imgs_path(self, args):
        self.list_hr, self.list_lr = self._scan()
        print('Loading images...')
        self.images_hr = []
        self.images_lr = []
        for i in range(len(self.list_hr)):
            imagei = cv2.cvtColor(cv2.imread(self.list_hr[i]), cv2.COLOR_BGR2RGB)
            self.images_hr.append(imagei)
        for j in range(len(self.list_hr)):
            imagej = [cv2.cvtColor(cv2.imread(self.list_lr[idx_scale][j]), cv2.COLOR_BGR2RGB) for idx_scale in range(len(self.scale))]
            self.images_lr.append(imagej)
    ...
    def _load_file(self, idx):
        ...
        f_lr = [self.images_lr[idx][idx_scale] for idx_scale in range(len(self.scale))]

        filename, _ = os.path.splitext(os.path.basename(self.list_hr[idx]))
        '''hr = cv2.imread(f_hr)
        hr = cv2.cvtColor(hr, cv2.COLOR_BGR2RGB)
        lr = [cv2.cvtColor(cv2.imread(f_lr[idx_scale]), cv2.COLOR_BGR2RGB) for idx_scale in range(len(self.scale))]'''
        return f_lr, f_hr, filename
